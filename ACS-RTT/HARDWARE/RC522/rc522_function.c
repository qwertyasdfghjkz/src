...
char ReadCardID(uint8_t *ID) 
{
	uint8_t ucStatusReturn;      /*返回状态*/
	/*寻卡*/
	if ( ( ucStatusReturn = PcdRequest ( PICC_REQALL, ID ) ) != MI_OK )
	{
			return 0;
		/*若失败再次寻卡*/
		//printf ( "寻卡失败\n" );
		//ucStatusReturn = PcdRequest ( PICC_REQALL, ID ); //PICC_REQALL   PICC_REQIDL
	}
...
{
	PcdAuthState( PICC_AUTHENT1A, 3, KeyValue, ID );//校验密码 
	PcdReadString(1,ICDate);
	if(ICPassswd[0]==ICDate[0]) 
	{
		ICDate[0]=0;
		return 1;
	}
...
void Verify_IC_Card(void)
{
	...
	if(ReadCardID(ID))
	{
		sprintf ( cStr, "ID:%02X%02X%02X%02X",ID [0], ID [1], ID [2],ID [3] );
		LCD_Fill(0,0,lcddev.width,139,WHITE);					
		LCD_ShowString(20,80,240,16,16,(u8*)cStr);
		
		rootMenuFlag=1;
		
		if(Check_IC_Card(ID))
		{
				LCD_ShowString(20,100,240,16,16,(u8*)"Correct");
				ServoControl(185);
				ServoFlag=1;
				delay_ms(1000);
				LCD_Fill(0,0,lcddev.width,139,WHITE);				
		...
		{
				LCD_ShowString(20,100,240,16,16,(u8*)"Faild");
				delay_ms(1000);
				LCD_Fill(0,0,lcddev.width,139,WHITE);
		}
	}
...
void Add_IC_Card(void)
{
	...
	while(1)
	{
		...
		if(ReadCardID(ID))
		{
			...
			LCD_ShowString(20,80,240,16,16,(u8*)cStr);
			PcdAuthState( PICC_AUTHENT1A, 3, KeyValue, ID );//校验密码 
			PcdWrite(1,ICPassswd);
			flag=1;
			delay_ms(50);
		}
			
		if(flag && Check_IC_Card(ID))
		{
			LCD_ShowString(20,100,240,16,16,(u8*)"Add successfully");
			flag=0;
			delay_ms(1000);
		}
		else if(flag)
		{
			LCD_ShowString(20,100,240,16,16,(u8*)"Add failed");
			flag=0;
			delay_ms(500);
		}	
...
void Del_IC_Card(void)
{
	...
	while(1)
	{
		...
		if(flag && !Check_IC_Card(ID))
		{
			LCD_ShowString(20,100,240,16,16,(u8*)"Deleted successfully");
			flag=0;
			delay_ms(1000);
		}
		else if(flag)
		{
			LCD_ShowString(20,100,240,16,16,(u8*)"Delete failed");
			flag=0;
			delay_ms(500);
		}	
